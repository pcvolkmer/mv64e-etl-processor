...
Dazu ist eine Konfiguration des in dieser Version eingeführten [Switch-Mechanismus](#switched-rest) erforderlich
...
Seit Version 0.18 ist eine Konfiguration des neu eingeführten **Switch**-Mechanismus möglich.
Damit kann für bestimmte Entitäten und Diagnosen ein abweichender REST-Endpunkt definiert werden.

Beispiel:

* `APP_SWITCH_NNGM_URI`: URI der nNGM-REST-API.
* `APP_SWITCH_NNGM_API_KEY`: API-Key zur Nutzung der nNGM-REST-API.
* `APP_SWITCH_NNGM_ICD10[0]`: Erster ICD-Code - weitere möglich.
...
Wird zum Beispiel `APP_SWITCH_NNGM_ICD10[0] = C34` konfiguriert, werden Diagnosen wie "C34.0", "C34.1", etc. an das nNGM
übermittelt.
