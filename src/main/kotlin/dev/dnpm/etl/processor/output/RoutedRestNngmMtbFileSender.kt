...
import dev.dnpm.etl.processor.config.RoutingProperties
...
class RoutedRestNngmMtbFileSender(
    private val restTemplate: RestTemplate,
    private val routingProperties: RoutingProperties,
    private val retryTemplate: RetryTemplate,
    private val reportService: ReportService,
) : RoutedMtbFileSender {
    private val logger = LoggerFactory.getLogger(RoutedRestNngmMtbFileSender::class.java)

    fun sendUrl(): String =
        UriComponentsBuilder
            .fromUriString(routingProperties.nngm?.uri.toString())
            .toUriString()
    ...
    override fun supportsDiagnosis(diagnosis: MtbDiagnosis): Boolean {
        ...
        this.routingProperties.nngm?.icd10?.forEach {
    ...
    override fun send(request: MtbFileRequest<PatientRecord>): MtbFileSender.Response {
        try {
            return retryTemplate.execute<MtbFileSender.Response, Exception> {
                ...
                logger.debug("Sent file via RoutedRestNngmMtbFileSender")
                return@execute MtbFileSender.Response(
                    reportService.deserialize(response.body).asRequestStatus(),
            ...
            logger.error("Not a valid URI to export to: '{}'", routingProperties.nngm?.uri!!)
        } catch (e: RestClientResponseException) {
            logger.info(routingProperties.nngm?.uri!!)
    ...
    fun endpoint(): String =
        this.routingProperties.nngm
            ?.uri
            .orEmpty()

    private fun getHttpHeaders(): HttpHeaders {
        val apiKey = routingProperties.nngm?.apiKey
